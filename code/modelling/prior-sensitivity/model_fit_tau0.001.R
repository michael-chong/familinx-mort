...
data_list <- list(
  n_countries = length(c(country_ref, country_unk)),
...
mod <- cmdstan_model(here("code/modelling/model65.stan"), pedantic = TRUE)
...
fit <- mod$sample(
  ...
  output_basename = "model65_tau0.001"
)
...
write_rds(
  ...
  file = here("output/intermediate/model65_tau0.001.rds")
)
